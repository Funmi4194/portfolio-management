...
export function mapToQuery(m: SQLMap, noArgs: boolean = false, dialect?: string): SQLQueryResult {
    let query = '';
    let args: any[] = [];
    ...
    for (const [k, v] of Object.entries(m.map)) {
        ...
        if (v instanceof SQLValueMerge) {
            if (dialect && dialect === 'mysql') {
                let col = k.includes('.') ? k.split('.')[1] : k;
                query += `${col} = ${col} ${v.operator} VALUES(${col}) ${m.joinOperator} `;
                continue;
            } else {
                let q = '';
...
export function mapsToIQuery(m: SQLMaps, dialect: string = 'mysql'): SQLQueryResult {
    let query = '';
    let subquery = '';
    let args: any[] = [];
        ...
        if (m.conflict?.length && m.sMap) {
            if (dialect === 'postgres') {
                ...
                if (!Object.keys(m.sMap.map).length) {
                    ...
                    const { query: squery, args: sargs } = mapToQuery(m.sMap, false, dialect);
                    query += squery;
                    args.push(...sargs);
                }
            }
        } else if (m.sMap && Object.keys(m.sMap.map).length) {
            query += dialect === 'postgres' ? ` ON CONFLICT DO UPDATE SET ` : ` ON DUPLICATE KEY UPDATE `;
            const { query: squery, args: sargs } = mapToQuery(m.sMap, false, dialect);
            query += squery;
            args.push(...sargs);
        }
