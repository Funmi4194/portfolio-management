...
export class Xiao<T> {
    ...
    /**
     * dialect is the type of sql database
     * @default 'postgres'
     */
    ...
    async create(m: SQLMaps): Promise<void> {
        ...
        await this.pool.query(
            parameterize(`${SQLOperator.INSERT} ${SQLOperator.INTO} ${this.tableName} ${query}`),
            args,
        );
    }
    ...
    async createTx(client: XiaoTx, m: SQLMaps): Promise<void> {
        const { query, args } = mapsToIQuery(m);
        await client.query(parameterize(`${SQLOperator.INSERT} ${SQLOperator.INTO} ${this.tableName} ${query}`), args);
    }
     ...
     */
    async findByKeyVal<K = T>(key: string, val: any, preload: boolean = true): Promise<T | K> {
     ...
     */
    async findAllByKeyVal<K = T>(key: string, val: any, preload: boolean = true): Promise<T[] | K[]> {
        const selectColumns = preload ? '*' : this.preloaders.join(', ');
        const { rows } = await this.pool.query(
            parameterize(
                `${SQLOperator.SELECT} ${selectColumns} ${SQLOperator.FROM} ${this.tableName} ${SQLOperator.WHERE} ${key} = $1`,
     ...
     * @returns the record found
     */
    async findAndLockByKeyVal<K = T>(client: XiaoTx, key: string, val: any, preload: boolean = true): Promise<T | K> {
        const selectColumns = preload ? '*' : this.preloaders.join(', ');
     ...
     * @returns the record found
     */
    async findByMap<K = T>(m: SQLMaps, preload: boolean = true): Promise<T | K> {
     ...
     * @returns an array of records found
     */
    async findAllByMap<K = T>(m: SQLMaps, preload: boolean = true): Promise<T[] | K[]> {
        const { query, args } = mapsToWQuery(m);
     ...
     * @returns the record found
     */
    async findAndLockByMap<K = T>(client: XiaoTx, m: SQLMaps, preload: boolean = true): Promise<T | K> {
        const { query, args } = mapsToWQuery(m);
        const selectColumns = preload ? '*' : this.preloaders.join(', ');
        const { rows } = await client.query(
            parameterize(
     ...
     * @returns an array of records found
     */
    async findAllAndLockByMap<K = T>(client: XiaoTx, m: SQLMaps, preload: boolean = true): Promise<T[] | K[]> {
        const { query, args } = mapsToWQuery(m);
        const oquery = mapsToOQuery(m);
        const lquery = mapsToLQuery(m);
        const selectColumns = preload ? '*' : this.preloaders.join(', ');
    ...
    async updateByMap<K = T>(m: SQLMaps): Promise<T[] | K[] | void> {
        ...
        if (query.includes(SQLOperator.RETURNING)) {
            const { rows } = await this.pool.query(
                parameterize(`${SQLOperator.UPDATE} ${this.tableName} ${query}`),
                args,
            );
            return rows as T[] | K[];
        } else {
            await this.pool.query(parameterize(`${SQLOperator.UPDATE} ${this.tableName} ${query}`), args);
        }
    }
    ...
    async updateByMapTx<K = T>(client: XiaoTx, m: SQLMaps): Promise<T[] | K[] | void> {
        const { query, args } = mapsToSQuery(m);
        if (query.includes(SQLOperator.RETURNING)) {
            const { rows } = await client.query(parameterize(`${SQLOperator.UPDATE} ${this.tableName} ${query}`), args);
            return rows as T[] | K[];
        } else {
            await client.query(parameterize(`${SQLOperator.UPDATE} ${this.tableName} ${query}`), args);
        }
    }

    /**
     * bulkUpdateByMap updates multiple records in the table by a map of conditions
     * @param m is a map of conditions to check
     * @returns the updated records
     */
    async bulkUpdateByMap<K = T>(m: SQLMaps[]): Promise<void> {
        let fquery = '';
        const fargs = [];
        for (const map of m) {
            const { query, args } = mapsToSQuery(map);
            fquery += `${SQLOperator.UPDATE} ${this.tableName} ${query}`;
            fargs.push(...args);
        }
        await this.pool.query(parameterize(fquery), fargs);
    }
    ...
    async countByMap(m: SQLMaps): Promise<number> {
        ...
        const { rows } = await this.pool.query(
            parameterize(
                `${SQLOperator.SELECT} ${SQLOperator.COUNT('*')} ${SQLOperator.AS} count ${SQLOperator.FROM} ${
                    this.tableName
                } ${SQLOperator.WHERE} ${query}`,
            ),
            args,
        );
    ...
    async deleteByMap(m: SQLMaps): Promise<void> {
        const { query, args } = mapsToWQuery(m);
        await this.pool.query(
            parameterize(`${SQLOperator.DELETE} ${SQLOperator.FROM} ${this.tableName} ${SQLOperator.WHERE} ${query}`),
            args,
        );
    }
    ...
    async execute<K = T>(query: string, args: any[]): Promise<T | T[] | K | K[]> {
        const { rows } = await this.pool.query(parameterize(query), args);
        if (rows.length === 1) {
            return rows?.[0] as T | K;
        }
        return rows as T[] | K[];
    ...
    async executeTx<K = T>(client: XiaoTx, query: string, args: any[]): Promise<T | T[] | K | K[]> {
        const { rows } = await client.query(parameterize(query), args);
        if (rows.length === 1) {
            return rows?.[0] as T | K;
        }
        return rows as T[] | K[];
